...
public class FileReaderWriter implements IOStreams {
	...
	private static final String FILEPATH = "src\\main\\java\\by\\epam\\cdp\\java\\nypresent\\storage\\present.txt";
			...
			while ((line = br.readLine()) != null) {
				...
				if (matcher.find()) {
					...
					if (candyType.equals(TypeOfCandy.BAR)) {
						present.addCandy(new Bar(name, weight, mainIngredient));
					} else if (candyType.equals(TypeOfCandy.CHOCOLATE)) {
						present.addCandy(new ChocolateCandy(name, weight, mainIngredient));
					} else if (candyType.equals(TypeOfCandy.LOLLYPOP)) {
						present.addCandy(new Lollypop(name, weight, mainIngredient));
					} else {
